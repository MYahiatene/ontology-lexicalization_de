...
my $CFG = {
    ...
    rulepattern                => {
        ...
        predict_p_for_o_given_l            => 0,
        predict_localized_l_for_o_given_p  => 0,
        predict_p_for_o_given_localized_l  => 0,
    },
...
my $entity_to_frequent_classes_file = "$BASEDIR/inter/entity_to_frequent_classes-" . $CFG->{min_entities_per_class} . "-" . $CFG->{max_entities_per_class} . ".yml";
my $step1_time_file = "$BASEDIR/inter/step1-" . $CFG->{min_entities_per_class} . "-" . $CFG->{max_entities_per_class} . ".time";
if (
...
my $step2_finished_file = "$BASEDIR/inter/step2-" . join("-",
    $CFG->{min_entities_per_class},
    $CFG->{max_entities_per_class},
    $CFG->{min_anchor_count}
) . ".finished";

my $step2_time_file = "$BASEDIR/inter/step2-" . join("-",
    $CFG->{min_entities_per_class},
    $CFG->{max_entities_per_class},
...
if (not -e $step2_finished_file) {
    ...
    my $zha = IO::Uncompress::Bunzip2->new(
        "$BASEDIR/input/inputSemantic/anchor-texts-sorted-counted-reversed.txt.bz2",
        { AutoClose => 1, Transparent => 1, }
    ) or die "IO::Uncompress::Bunzip2 failed: $Bunzip2Error\n";
    while (my $line = <$zha>) {
        if ($line =~ m/(\d+) (.*)\t(.*)\n/) {
    ...
    # TODO:check this complete for each
    foreach my $e (keys %{$entity_to_pos_to_triples}) {
        ...
        mkdir "$BASEDIR/inter/data_per_entity/$last/" if not -d "$BASEDIR/inter/data_per_entity/$last/";
        foreach my $pos (keys %{$entity_to_pos_to_triples->{$e}}) {
            # TODO take care of filenames that are too long

            my $entitydatafilename = "$BASEDIR/inter/data_per_entity/$last/$e_enc-$pos-" . $CFG->{min_anchor_count} . ".ttl";
    ...
    foreach my $c (keys %{$class_to_pos_to_triples}) {
        ...
        mkdir "$BASEDIR/inter/data_per_class/$c_enc/" if not -d "$BASEDIR/inter/data_per_class/$c_enc";
        foreach my $pos (qw(sub obj)) {
            ...
            #print " > $BASEDIR/inter/data_per_class/$c_enc/$pos-" . $CFG->{min_entities_per_class} . "-" . $CFG->{max_entities_per_class} . ".ttl\n";
            open(DAT, ">$BASEDIR/inter/data_per_class/$c_enc/$pos-" . $CFG->{min_entities_per_class} . "-" . $CFG->{max_entities_per_class} . ".ttl");
            foreach my $line (@{$class_to_pos_to_triples->{$c}->{$pos}}) {
                print DAT $line;
            }
            close DAT;
            unlink "$BASEDIR/inter/data_per_class/$c_enc/$pos-" . $CFG->{min_entities_per_class} . "-" . $CFG->{max_entities_per_class} . ".ttl.bz2"
                if -e "$BASEDIR/inter/data_per_class/$c_enc/$pos-" . $CFG->{min_entities_per_class} . "-" . $CFG->{max_entities_per_class} . ".ttl.bz2";
            system("bzip2 $BASEDIR/inter/data_per_class/$c_enc/$pos-" . $CFG->{min_entities_per_class} . "-" . $CFG->{max_entities_per_class} . ".ttl");
        }
    }
...
my $step3_finished_file = "$BASEDIR/inter/step3-" . join("-",
    $CFG->{min_entities_per_class},
    $CFG->{max_entities_per_class},
...
my $step3_time_file = "$BASEDIR/inter/step3-" . join("-",
    $CFG->{min_entities_per_class},
    $CFG->{max_entities_per_class},
    $CFG->{min_onegram_length},
    $CFG->{min_anchor_count},
...
if (not -e $step3_finished_file) {
    ...
    while (my $line = <$zh>) {
        ...
        if (exists $entity_to_frequent_classes->{$e}) {
            ...
            my $last = substr($e_enc, -$folder_length);
            my $propertypatternfilename = "$BASEDIR/inter/data_per_entity/$last/$e_enc-propertypatterns-" .
                join("-",
                    $CFG->{min_entities_per_class},
                    $CFG->{max_entities_per_class},
                    $CFG->{min_onegram_length},
            ...
            #new line of code
            my $propertypatternfilenamelength = length($propertypatternfilename) + 4 - length("$BASEDIR/inter/data_per_entity/$last/"); # +4: ".bz2"
            ...
            my $patternfilename = "$BASEDIR/inter/data_per_entity/$last/$e_enc-patterns-" . join("-",
                $CFG->{min_entities_per_class},
                $CFG->{max_entities_per_class},
                $CFG->{min_onegram_length}) . ".yml";
            my $patternfilenamelength = length($patternfilename) + 4 - length("$BASEDIR/inter/data_per_entity/$last/"); # +4: ".bz2"
            ...
            if (-s "$entitysubdatafilename.bz2" or -s $entitysubdatafilename) {
                # TODO remove uncompressed processing
            ...
            if (-s "$entityobjdatafilename.bz2" or -s $entityobjdatafilename) {
                # TODO remove processing of uncompressed
            ...
            {
                my @matches;
                push(@matches, $1) while ($abstract =~ /\. Er /g);
                foreach my $match (@matches) {
                    $abstract =~ s/\. ER /. $entity_label /;
                    #print " 2> $abstract\n"; <STDIN>;
                }
            }
            {
                my @matches;
                push(@matches, $1) while ($abstract =~ /\. Sie /g);
                foreach my $match (@matches) {
                    $abstract =~ s/\. Sie /. $entity_label /;
                    #print " 3> $abstract\n"; <STDIN>;
                }
            }
            {
                my @matches;
                push(@matches, $1) while ($abstract =~ /\. Es /g);
                foreach my $match (@matches) {
                    $abstract =~ s/\. Es /. $entity_label /;
                    #print " 4> $abstract\n"; <STDIN>;
                }
            }
            {
                my @matches;
                push(@matches, $1) while ($abstract =~ /\. Sein /g);
                foreach my $match (@matches) {
                    $abstract =~ s/\. Sein /. $entity_label\'s /;
                    #print " 5> $abstract\n"; <STDIN>;
                }
            }
            {
                my @matches;
                push(@matches, $1) while ($abstract =~ /\. Ihr /g);
                foreach my $match (@matches) {
                    $abstract =~ s/\. Ihr /. $entity_label\'s /;
                    #print " 6> $abstract\n"; <STDIN>;
                }
            }
            {
            ...
            #	$CFG->{max_propertystring_length}
            #) . ".yml";
            #my $propertypatternfilenamelength = length($propertypatternfilename) +4 - length("$BASEDIR/input/data_per_entity/$last/"); # +4: ".bz2"
            ...
            #	$CFG->{max_entities_per_class},
            #	$CFG->{min_onegram_length}) . ".yml";
            #my $patternfilenamelength = lengt($patternfilename) +4 - length("$BASEDIR/inter/data_per_entity/$last/"); # +4: ".bz2"
...
# Step 4. collect frequent linguistic patterns for each frequent class
my $step4_finished_file = "$BASEDIR/inter/step4-" . join("-",
    $CFG->{min_entities_per_class},
    $CFG->{max_entities_per_class},
    $CFG->{min_onegram_length},
...
my $step4_time_file = "$BASEDIR/inter/step4-" . join("-",
    $CFG->{min_entities_per_class},
    $CFG->{max_entities_per_class},
    $CFG->{min_onegram_length},
    $CFG->{min_pattern_count},
...
if (-e $step3_finished_file and not -e $step4_finished_file) {
    ...
    foreach my $c (sort keys %{$frequent_class_to_entities}) {
        ...
        my $classpatternfilename = "$BASEDIR/inter/data_per_class/$c_enc/$c_enc-patterns-" . join("-",
            $CFG->{min_entities_per_class},
            $CFG->{max_entities_per_class},
            $CFG->{min_onegram_length},
            $CFG->{min_pattern_count}
        ...
        my $classpropertypatternfilename = "$BASEDIR/inter/data_per_class/$c_enc/$c_enc-propertypatterns-" . join("-",
            $CFG->{min_entities_per_class},
            $CFG->{max_entities_per_class},
            $CFG->{min_anchor_count},
            $CFG->{min_propertyonegram_length},
        ...
        if (not -s "$classpatternfilename.bz2" or not -s "$classpropertypatternfilename.bz2") {
            ...
            foreach my $e (keys %{$frequent_class_to_entities->{$c}}) {
                ...
                my $last = substr($e_enc, -$folder_length);
                my $entitypatternfilename = "$BASEDIR/inter/data_per_entity/$last/$e_enc-patterns-" . join("-",
                    $CFG->{min_entities_per_class},
                    $CFG->{max_entities_per_class},
                    $CFG->{min_onegram_length}
                ) . ".yml";

                if (-s "$entitypatternfilename.bz2") {
                    $cnt_found++;
...
my $step5_finished_file = "$BASEDIR/inter/step5-" . join("-",
    $CFG->{min_entities_per_class},
    $CFG->{max_entities_per_class},
    $CFG->{min_onegram_length},
    $CFG->{min_pattern_count},
...
my $step5_time_file = "$BASEDIR/inter/step5-" . join("-",
    $CFG->{min_entities_per_class},
    $CFG->{max_entities_per_class},
    $CFG->{min_onegram_length},
    $CFG->{min_pattern_count},
...
if (-e $step4_finished_file and not -e $step5_finished_file) {
    ...
    foreach my $c (sort keys %{$frequent_class_to_entities}) {
        ...
        my $patternfilename = "$BASEDIR/inter/data_per_class/$c_enc/$c_enc-patterns-" . join("-",
            $CFG->{min_entities_per_class},
            $CFG->{max_entities_per_class},
            $CFG->{min_onegram_length},
            $CFG->{min_pattern_count}
        ...
        my $propertypatternfilename = "$BASEDIR/inter/data_per_class/$c_enc/$c_enc-propertypatterns-" . join("-",
            $CFG->{min_entities_per_class},
            $CFG->{max_entities_per_class},
            $CFG->{min_anchor_count},
            $CFG->{min_propertyonegram_length},
        ...
        if (
            ...
            my $datafilename = "$BASEDIR/inter/data_per_class/$c_enc/sub-" . $CFG->{min_entities_per_class} . "-" . $CFG->{max_entities_per_class} . ".ttl";
            ...
            if ($CFG->{rulepattern}->{predict_l_for_s_given_po} or $CFG->{rulepattern}->{predict_po_for_s_given_l}) {
                ...
                my $filename1_csv = "$BASEDIR/results/rules-predict_l_for_s_given_po-$parameterstring_nonlocalized.csv";
                my $filename2_csv = "$BASEDIR/results/rules-predict_po_for_s_given_l-$parameterstring_nonlocalized.csv";
            ...
            if ($CFG->{rulepattern}->{predict_localized_l_for_s_given_po} or $CFG->{rulepattern}->{predict_po_for_s_given_localized_l}) {
                ...
                my $filename1_csv = "$BASEDIR/results/rules-predict_localized_l_for_s_given_po-$parameterstring_localized.csv";
                my $filename2_csv = "$BASEDIR/results/rules-predict_po_for_s_given_localized_l-$parameterstring_localized.csv";
                my $csv1 = Text::CSV->new({ binary => 1, auto_diag => 1 });
                my $csv2 = Text::CSV->new({ binary => 1, auto_diag => 1 });
                my ($fh1, $fh2);
            ...
            if ($CFG->{rulepattern}->{predict_l_for_s_given_p} or $CFG->{rulepattern}->{predict_p_for_s_given_l}) {
                ...
                my $filename1_csv = "$BASEDIR/results/rules-predict_l_for_s_given_p-$parameterstring_nonlocalized.csv";
                my $filename2_csv = "$BASEDIR/results/rules-predict_p_for_s_given_l-$parameterstring_nonlocalized.csv";
                my $csv1 = Text::CSV->new({ binary => 1, auto_diag => 1 });
                my $csv2 = Text::CSV->new({ binary => 1, auto_diag => 1 });
                my ($fh1, $fh2);
            ...
            if ($CFG->{rulepattern}->{predict_localized_l_for_s_given_p} or $CFG->{rulepattern}->{predict_p_for_s_given_localized_l}) {
                ...
                my $filename1_csv = "$BASEDIR/results/rules-predict_localized_l_for_s_given_p-$parameterstring_localized.csv";
                my $filename2_csv = "$BASEDIR/results/rules-predict_p_for_s_given_localized_l-$parameterstring_localized.csv";
                my $csv1 = Text::CSV->new({ binary => 1, auto_diag => 1 });
                my $csv2 = Text::CSV->new({ binary => 1, auto_diag => 1 });
                my ($fh1, $fh2);
            ...
            if ($CFG->{rulepattern}->{predict_l_for_s_given_o} or $CFG->{rulepattern}->{predict_o_for_s_given_l}) {
                ...
                my $filename1_csv = "$BASEDIR/results/rules-predict_l_for_s_given_o-$parameterstring_nonlocalized.csv";
                my $filename2_csv = "$BASEDIR/results/rules-predict_o_for_s_given_l-$parameterstring_nonlocalized.csv";
                my $csv1 = Text::CSV->new({ binary => 1, auto_diag => 1 });
                my $csv2 = Text::CSV->new({ binary => 1, auto_diag => 1 });
                my ($fh1, $fh2);
        ...
        if (
            ...
            my $datafilename = "$BASEDIR/inter/data_per_class/$c_enc/obj-" . $CFG->{min_entities_per_class} . "-" . $CFG->{max_entities_per_class} . ".ttl";
            next FEC if not -s "$datafilename.bz2";
            ...
            if ($CFG->{rulepattern}->{predict_l_for_o_given_sp} or $CFG->{rulepattern}->{predict_sp_for_o_given_l}) {
                ...
                my $filename1_csv = "$BASEDIR/results/rules-predict_l_for_o_given_sp-$parameterstring_nonlocalized.csv";
                my $filename2_csv = "$BASEDIR/results/rules-predict_sp_for_o_given_l-$parameterstring_nonlocalized.csv";
                my $csv1 = Text::CSV->new({ binary => 1, auto_diag => 1 });
                my $csv2 = Text::CSV->new({ binary => 1, auto_diag => 1 });
                my ($fh1, $fh2);
            ...
            if ($CFG->{rulepattern}->{predict_localized_l_for_o_given_sp} or $CFG->{rulepattern}->{predict_sp_for_o_given_localized_l}) {
                ...
                my $filename1_csv = "$BASEDIR/results/rules-predict_localized_l_for_o_given_sp-$parameterstring_localized.csv";
                my $filename2_csv = "$BASEDIR/results/rules-predict_sp_for_o_given_localized_l-$parameterstring_localized.csv";
                my $csv1 = Text::CSV->new({ binary => 1, auto_diag => 1 });
                my $csv2 = Text::CSV->new({ binary => 1, auto_diag => 1 });
                my ($fh1, $fh2);
            ...
            if ($CFG->{rulepattern}->{predict_l_for_o_given_s} or $CFG->{rulepattern}->{predict_s_for_o_given_l}) {
                ...
                my $filename1_csv = "$BASEDIR/results/rules-predict_l_for_o_given_s-$parameterstring_nonlocalized.csv";
                my $filename2_csv = "$BASEDIR/results/rules-predict_s_for_o_given_l-$parameterstring_nonlocalized.csv";
                my $csv1 = Text::CSV->new({ binary => 1, auto_diag => 1 });
                my $csv2 = Text::CSV->new({ binary => 1, auto_diag => 1 });
                my ($fh1, $fh2);
            ...
            if ($CFG->{rulepattern}->{predict_l_for_o_given_p} or $CFG->{rulepattern}->{predict_p_for_o_given_l}) {
                ...
                my $filename1_csv = "$BASEDIR/results/rules-predict_l_for_o_given_p-$parameterstring_nonlocalized.csv";
                my $filename2_csv = "$BASEDIR/results/rules-predict_p_for_o_given_l-$parameterstring_nonlocalized.csv";
                my $csv1 = Text::CSV->new({ binary => 1, auto_diag => 1 });
                my $csv2 = Text::CSV->new({ binary => 1, auto_diag => 1 });
                my ($fh1, $fh2);
            ...
            if ($CFG->{rulepattern}->{predict_localized_l_for_o_given_p} or $CFG->{rulepattern}->{predict_p_for_o_given_localized_l}) {
                ...
                my $filename1_csv = "$BASEDIR/results/rules-predict_localized_l_for_o_given_p-$parameterstring_localized.csv";
                my $filename2_csv = "$BASEDIR/results/rules-predict_p_for_o_given_localized_l-$parameterstring_localized.csv";
                my $csv1 = Text::CSV->new({ binary => 1, auto_diag => 1 });
                my $csv2 = Text::CSV->new({ binary => 1, auto_diag => 1 });
                my ($fh1, $fh2);
